...
class SmartOnionDateEncoder(Encoder):
  """
  ...
  def __init__(self, season=0, dayOfWeek=0, weekend=0, holiday=5, timeOfDay=0, customDays=0,
                name='', forced=True):
  ...
  def getEncodedValues(self, input):
    ...
    # -------------------------------------------------------------------------
    # Get the scalar values for each sub-field
    greg_timetuple = input.timetuple()
    heb_date = pyluach.hebrewcal.HebrewDate.from_pydate(input)
    last_rosh_hashana = pyluach.hebrewcal.HebrewDate(heb_date.year, 7, 1)
    heb_date_dict = heb_date.dict()
    timedict = {
      "tm_year": heb_date_dict["year"],
      "tm_mon": heb_date_dict["month"],
      "tm_mday": heb_date_dict["day"],
      "tm_hour": greg_timetuple.tm_hour,
      "tm_min": greg_timetuple.tm_min,
      "tm_sec": greg_timetuple.tm_sec,
      "tm_wday": greg_timetuple.tm_wday,
      "tm_yday": heb_date - last_rosh_hashana,
      "tm_isdst": greg_timetuple.tm_isdst
    }
    timeOfDay = timedict["tm_hour"] + float(timedict["tm_min"]) / 60.0

    if self.seasonEncoder is not None:
      dayOfYear = timedict["tm_yday"]
      # input.timetuple() computes the day of year 1 based, so convert to 0 based
      values.append(dayOfYear-1)

    if self.dayOfWeekEncoder is not None:
      dayOfWeek = timedict["tm_wday"] + timeOfDay / 24.0
      values.append(dayOfWeek)

    if self.weekendEncoder is not None:
      # saturday, sunday or friday evening
      if timedict["tm_wday"] == 4 or timedict["tm_wday"] == 5:
        weekend = 1
      else:
        weekend = 0
    ...
    if self.customDaysEncoder is not None:
      if timedict["tm_wday"] in self.customDays:
        customDay = 1
      else:
        customDay = 0
    ...
    if self.holidayEncoder is not None:
      ...
      val = 0
      if pyluach.hebrewcal.holiday(heb_date, israel=True):
        hdate = datetime.datetime(greg_timetuple.tm_year, greg_timetuple.tm_mon, greg_timetuple.tm_mday, 0, 0, 0)
        if input > hdate:
          diff = input - hdate
          if diff.days == 0:
